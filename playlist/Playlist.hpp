...
private:
	...
	bool doesFind(const Entry &e);
	Node<Entry>* findFromTitle(const std::string &_title);
	void swap(Node<Entry>* ref, Node<Entry>* min);
	void callInsert(string title, string genre, string year);
public:
	Playlist();
	...
	void printHistory();
	Node<Entry>* getPointer() const;
	void insertByIndex(int index, Entry &e);
	Node<Entry>* findFromIndex(int index);
